...
DB_FILE="tpcds_sf100.duckdb"
QUERIES_DIR="duckdb/extension/tpcds/dsdgen/queries"
OUTPUT_FILE="project_results/data/tpcds_sf100.txt"
...
{
echo "======================================="
echo "TPC-DS SF100: CPU vs GPU (First $MAX_QUERIES queries)"
echo "Date: $(date)"
echo "======================================="
echo ""
...
echo "======================================="
printf "%-10s %10s %10s %10s %10s %12s\n" "Query" "CPU(ms)" "GPU(ms)" "Speedup" "Saved(ms)" "Match"
echo "----------------------------------------------------------------------"
...
while IFS= read -r line; do
    if [[ $line == *"QUERY_START:"* ]]; then
        ...
        time_saved=$((cpu_time_ms - gpu_time_ms))
        ...
        printf "%-10s %10d %10d %10s %10s %12s\n" "$current_query" "$cpu_time_ms" "$gpu_time_ms" "${speedup}x" "$time_saved" "$match"
